...
class Instance:
    ...
    def __solve_fptas(self, eps):
        ...
        print k
        height = int(sum(categorized_costs))
        print float(height)
        ...
        while 0 != len(node_queue):
            ...
            if (node[1] <= self.N - 1):
                ...
                if node[0] + categorized_costs[node[1]] <= height:
                    next_node = (int(node[0] + categorized_costs[node[1]]), node[1] + 1)
...
def main(argc, argv):
    ...
    print("Total runtime: {}, avg runtime: {}.".format(total_runtime, average_runtime))

    if 'fptas' == method:
        average_relative_error = total_relative_error / len(instances)
        print("Maximum relative error: {}, avg relative error: {}.".format(maximum_relative_error, average_relative_error))
